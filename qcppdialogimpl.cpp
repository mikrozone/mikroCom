...
,m_previousChar('\0')
{
   QCoreApplication::setOrganizationName("mikroCom [mikroZone]");
   QCoreApplication::setOrganizationDomain("mikrozone.eu");
   QCoreApplication::setApplicationName("mikroCom [mikroZone]");
...
{
   QSettings settings;
   settings.setValue("/mikroCom/HardwareHandshake", m_hardwareCb->isChecked());
   settings.setValue("/mikroCom/SoftwareHandshake", m_softwareCb->isChecked());

   settings.setValue("/mikroCom/OpenForReading", m_readCb->isChecked());
   settings.setValue("/mikroCom/OpenForWriting", m_writeCb->isChecked());
   settings.setValue("/mikroCom/DontApplySettings", !m_applyCb->isChecked());

   settings.setValue("/mikroCom/Device", m_deviceCb->currentText());
   settings.setValue("/mikroCom/Baud", m_baudCb->currentItem());
   settings.setValue("/mikroCom/Databits", m_dataBitsCb->currentItem());
   settings.setValue("/mikroCom/Parity", m_parityCb->currentItem());
   settings.setValue("/mikroCom/Stopbits", m_stopCb->currentItem());
   settings.setValue("/mikroCom/Protocol", m_protoPb->currentItem());
   settings.setValue("/mikroCom/width", width());
   settings.setValue("/mikroCom/height", height());

   settings.setValue("/mikroCom/LineMode", m_inputModeCb->currentItem());
   settings.setValue("/mikroCom/HexOutput", m_hexOutputCb->isChecked());
   settings.setValue("/mikroCom/CharDelay", m_charDelaySb->value());

   settings.setValue("/mikroCom/SendFileDialogStartDir", m_sendFileDialogStartDir);
   settings.setValue("/mikroCom/LogFileName", m_logFileLe->text());

   settings.setValue("/mikroCom/AppendToLogFile", m_logAppendCb->currentItem());

   // EdizonTN **********************
   settings.setValue("/mikroCom/RTS", m_RTS->isChecked());
   settings.setValue("/mikroCom/DTR", m_DTR->isChecked());
   // *******************************

   QString currentDevice=m_deviceCb->currentText();
   settings.setValue("/mikroCom/CurrentDevice", currentDevice);
   ...
   settings.setValue("/mikroCom/AllDevices", devices);
   ...
   settings.setValue("/mikroCom/History", saveHist);
}

void QCPPDialogImpl::readSettings()
{
   QSettings settings;
   m_hardwareCb->setChecked(settings.value("/mikroCom/HardwareHandshake", false).toBool());
   m_softwareCb->setChecked(settings.value("/mikroCom/SoftwareHandshake", false).toBool());
   m_readCb->setChecked(settings.value("/mikroCom/OpenForReading", true).toBool());
   m_writeCb->setChecked(settings.value("/mikroCom/OpenForWriting", true).toBool());
   // EdizonTN **************************
   m_RTS->setChecked(settings.value("/mikroCom/RTS", false).toBool());
   m_DTR->setChecked(settings.value("/mikroCom/DTR", false).toBool());
   // ***********************************
   m_applyCb->setChecked(!settings.value("/mikroCom/DontApplySettings", false).toBool());
   enableSettingWidgets(m_applyCb->isChecked());

   int defaultBaud = settings.value("/mikroCom/Baud", -1).toInt();
   if (defaultBaud != -1)
   {
      m_baudCb->setCurrentIndex(defaultBaud);
   }
   m_dataBitsCb->setCurrentIndex(settings.value("/mikroCom/Databits", 3).toInt());
   m_parityCb->setCurrentIndex(settings.value("/mikroCom/Parity", 0).toInt());
   m_stopCb->setCurrentIndex(settings.value("/mikroCom/Stopbits", 0).toInt());
   m_protoPb->setCurrentIndex(settings.value("/mikroCom/Protocol", 0).toInt());

   m_inputModeCb->setCurrentIndex(settings.value("/mikroCom/LineMode", 0).toInt());
   m_hexOutputCb->setChecked(settings.value("/mikroCom/HexOutput", false).toBool());
   m_charDelaySb->setValue(settings.value("/mikroCom/CharDelay", 1).toInt());

   m_sendFileDialogStartDir=settings.value("/mikroCom/SendFileDialogStartDir", QDir::homePath()).toString();
   m_logFileLe->setText(settings.value("/mikroCom/LogFileName", QDir::homePath()+"/mikroCom.log").toString());

   m_logAppendCb->setCurrentIndex(settings.value("/mikroCom/AppendToLogFile", 0).toInt());

   int x=settings.value("/mikroCom/width", -1).toInt();
   int y=settings.value("/mikroCom/height", -1).toInt();
   if ((x>100) && (y>100))
   {
      resize(x,y);
   }

   bool entryFound = settings.contains("/mikroCom/AllDevices");
   QStringList devices=settings.value("/mikroCom/AllDevices").toStringList();
   if (!entryFound)
   {
      devices<<"/dev/ttyS0"<<"/dev/ttyS1"<<"/dev/ttyS2"<<"/dev/ttyS3"<<"/dev/ttyUSB0"<<"/dev/ttyUSB1";
   }

   m_deviceCb->insertItems(0, devices);

   int indexOfCurrentDevice = devices.indexOf(settings.value("/mikroCom/CurrentDevice", "/dev/ttyS0").toString());
   // fprintf(stderr, "currentDEev: -%s - index: %d\n", settings.value("/mikroCom/CurrentDevice", "/dev/ttyS0").toString().toLatin1().constData(), indexOfCurrentDevice);
   if (indexOfCurrentDevice!=-1)
   {
       m_deviceCb->setCurrentIndex(indexOfCurrentDevice);
   }

   QStringList history=settings.value("/mikroCom/History").toStringList();
...
//   cerr<<"readFromStdout() "<<ba.count()<<std::endl;
   ...
   {
      int bytesWritten=::write(m_fd, src, (bytesToWrite>COMM_BUFFSIZE?COMM_BUFFSIZE:bytesToWrite));
      if (bytesWritten<0)
      {
...
//EdizonTN: *****************
void QCPPDialogImpl::on_m_DTR_clicked(bool checked)
{
    int flag;
    if (m_isConnected == true )
    {
        ioctl(m_fd,TIOCMGET,&flag); /* GET the State of MODEM bits in Status */
        if(checked) flag |= TIOCM_DTR;        // Set the DTR pin
        else        flag &= ~TIOCM_DTR;       // Clear the DTR pin
        ioctl(m_fd, TIOCMSET, &flag);
    }
}

void QCPPDialogImpl::on_m_RTS_clicked(bool checked)
{
    int flag;
    if (m_isConnected == true )
    {
        ioctl(m_fd,TIOCMGET,&flag); /* GET the State of MODEM bits in Status */
        if (checked) flag |= TIOCM_RTS;        // Set the RTS pin
        else         flag &= ~TIOCM_RTS;       // Clear the RTS pin
        ioctl(m_fd, TIOCMSET, &flag);
    }
}
// **************************
